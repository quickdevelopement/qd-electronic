<div class="p-4 ">
    ...
    <table class="w-full  bg-white dark:bg-gray-800  rounded">
        <thead>
            <tr class="bg-green-800 dark:bg-gray-700 text-left text-white dark:text-gray-100">
                <th class="p-2">Title</th>
                <th class="p-2">SKU</th>
                <th class="p-2">Stock Quantity</th>
                <th class="p-2">Reserved Quantity</th>
                <th class="p-2">Low Stock Threshold</th>
        ...
        <tbody class="text-gray-700 dark:text-gray-300">
            ...
            <tr class="border-b dark:border-gray-700 border-gray-300 ">
                <td class="p-2">{{ $inventory->product->title }}</td>
                <td class="p-2">{{ $inventory->sku }}</td>
                <td class="p-2">{{ $inventory->stock_quantity }}</td>
                <td class="p-2">{{ $inventory->reserved_quantity }}</td>
                <td class="p-2">{{ $inventory->low_stock_threshold }}</td>
                <td class="p-2 flex space-x-2">
                    <a href="{{ route('dashboard.inventories.edit', $inventory->id) }}" class="text-yellow-600">
                    ...
                    <form action="{{ route('dashboard.inventories.delete', $inventory->id) }}" method="POST"
            ...
            @empty
            <tr>
                <td colspan="4" class="p-4 text-center">No inventories found</td>
            </tr>
            @endforelse
        </tbody>
    </table>
